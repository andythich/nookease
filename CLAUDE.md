...
20a. **Auto-advance is disabled on touch devices and that's load-bearing.** Originally the rAF loop ran on every device and only paused on hover, drag, or wheel-cooldown. On phones this was disastrous: `touch-action: pan-y` lets the browser handle horizontal swipes natively (good — that's what makes the carousel feel like a normal scroller), but React's synthetic `onPointerDown` doesn't always fire reliably during native scroll gestures, so `dragRef.current.active` would stay `false`. The user's finger flick would scroll the element, and a frame later the rAF loop would write `el.scrollLeft = virtualScrollRef.current` and yank it back. Net result: ~100 swipes to traverse 5 panels. Fix: `const isTouch = matchMedia('(hover: none)').matches` is added to the `isPaused` check in the tick loop. On touch, auto-advance never runs at all and the user's finger is the only thing moving scrollLeft. Don't re-enable auto-advance on touch — it cannot coexist with native horizontal scroll.
