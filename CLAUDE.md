...
| `carousel-scroller`    | The carousel scroll container   | Hides the scrollbar (`::-webkit-scrollbar`); `-webkit-overflow-scrolling: touch` for momentum scroll on iOS |
...
10a. **Journal characters disappearing while typing** — two distinct bugs that were both fixed; if either symptom returns, check these first:
   - **Editor sync effect comparing innerHTML to activePage.html.** The original effect ran `if (editorRef.current.innerHTML !== activePage.html) editorRef.current.innerHTML = activePage.html` whenever the effect ran. Browsers re-serialize contentEditable HTML (whitespace normalization, attribute reordering, `<br>` vs `</p><p>`), so `innerHTML !== activePage.html` is spuriously true on most renders. Any re-render during typing could overwrite the editor with stale state and wipe in-progress characters. Fix: a `lastSyncedIdRef` ref tracks which `activeId` was last written into the editor, and we only sync when `lastSyncedIdRef.current !== activeId`. Never go back to comparing innerHTML strings.
   - **Debounced save reading `activeId` at fire time, not schedule time.** The `setTimeout` callback closed over the React state `activeId`, which is read when the timer fires. Switch pages within the 600ms debounce window and the save would write your unfinished text to the wrong row — or the freshly-fetched page would hydrate into the editor before your last keystrokes saved, looking like character loss. Fix: `const idAtSchedule = activeId` is captured before `setTimeout`, and the `.eq('id', idAtSchedule)` uses the captured value. Don't refactor this back to read state inside the callback.
...
20a. **On touch devices the carousel runs an entirely separate code path: native scroll only.** Original behavior: rAF auto-advance + JS pointer-drag + wrap-snap, all running on every device. On phones this was broken in two ways that together produced the ~100-swipes-to-traverse symptom. The auto-advance disable (the original "20a fix") only addressed half of it, and the user-reported regression in 2026-05 made it clear:
   - **Half 1 — auto-advance fights native scroll.** `touch-action: pan-y` lets the browser handle horizontal swipes natively, but the rAF loop wrote `el.scrollLeft = virtualScrollRef.current` every frame and yanked the element back. *Earlier fix:* `isTouch = matchMedia('(hover: none)').matches` short-circuits the `isPaused` check so auto-advance never runs on touch.
   - **Half 2 — the JS pointer-drag handler has no momentum.** This is the part that was missed. `touch-action: pan-y` doesn't enable native horizontal scroll — it disables it (the `pan-y` allows only vertical pan; horizontal events go to JS). The JS `onPointerMove` handler then sets `el.scrollLeft = d.startScroll - dx`, which is a raw 1:1 finger-to-pixel drag with zero inertia. Every flick only travels as far as your finger physically moved. Combined with the wrap-around block (which writes scrollLeft *every frame* and interrupts any momentum that does occur), this is what produced the "100 swipes" feel.
   - **Current fix (full):** on touch, the carousel skips the rAF loop entirely (`if (isTouch) return cleanup` right after the initial scroll positioning), passes `undefined` for all pointer handlers in the JSX, and switches `touch-action` to `pan-x pan-y` so the browser handles horizontal scroll natively with iOS momentum. The 3-copy buffer (~5520px of slack) is enough that the user won't reach the hard edge in normal swiping; if they ever do, the carousel just stops, which is fine. Desktop behavior is unchanged.
   - **Don't undo any piece of this:** re-enabling auto-advance, re-attaching pointer handlers on touch, or reverting `touch-action` to `pan-y` will all bring back the clunky-mobile-scroll problem in some form.

20b. **Don't add `scroll-snap-type` to the carousel.** When fixing 20a, scroll-snap on `.carousel-scroller` was tried as a "while we're here" improvement to align swipes to panel boundaries. It made the carousel completely unswipable on iOS. Cause: the rAF tick loop's wrap-around block (the `if (el.scrollLeft >= third * 2)` check) runs *every frame, unconditionally*, including while the user is mid-gesture. With `scroll-snap-type: mandatory`, the browser's native snap logic and the rAF loop's `scrollLeft -= third` writes fight each other during the touch, and iOS Safari resolves the conflict by cancelling the gesture. Fixing this properly would require gating the wrap-around on touch and re-implementing it via the `scrollend` event (patchy support, especially older iOS). Not worth it — the auto-advance disable from 20a alone solves the original "100 swipes" problem, native momentum scroll feels fine without snap. **If a future change wants snap behavior, the wrap-around logic has to be rewritten first; don't just add the CSS.**
